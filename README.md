...
1. **默认随机高位端口（零冲突）**：默认不占用 80/443 端口，安装时自动检测并分配系统空闲随机端口（20000-55000，回车即用），与宿主机原有 Nginx、宝塔、1Panel、Xray/V2Ray 节点完美共存，零中断零冲突！
2. **支持 Caddy 自动 HTTPS**：对于纯净 VPS，亦可自主选择独占模式，由内置 Caddy 2 自动申请与续期 Let's Encrypt / ZeroSSL 证书。
3. **独家 Gemini 适配层**：
   - 自动修复客户端（Codex Desktop、ZCode 等）调用 Gemini 工具时报 `400 Unknown name "const" / "anyOf"` 的兼容性缺陷。
   - 自动拦截流式/非流式响应中的 `<thinking>` 标签，无缝转换为标准 `reasoning_content`。
4. **数据库与安全隔离**：PostgreSQL 16 与 Redis 7 仅在容器专用内网运行，不暴露宿主机危险端口。
5. **全功能运维面板**：包含一键安装、日志监控、在线升级、一键换域名与证书重签、管理员密码重置、全量打包备份、一键干净卸载。
